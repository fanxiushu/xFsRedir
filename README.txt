...
xFsRedir是集成多种功能于一身的复合型程序。
  ...
  可以是磁盘模拟器，把网络服务器上的磁盘通过各种通讯协议映射成本地磁盘。
  可以构建虚拟局域网，实现不同局域网内的设备可以互相通信。
  可以实现网络代理功能，让不支持代理的程序或者设备也能通过代理上网。
  xFsRedir包括四大部分功能：
   一，核心的功能是通过各种通讯协议把远程目录或文件映射成本地目录或文件。
   二，通过各种通讯协议把远程镜像文件或磁盘映射成本地磁盘。
   三，虚拟局域网，包括虚拟局域网节点和桥接到真实局域网。
   四，网络代理，包括NAT网关代理和本机代理。
...
程序使用说明以及开放源代码说明：
    ...
    4.1以上的版本增加了桌面水印，同时每个监控目录下增加“范秀树.xFsRedir.说明.txt”版权说明文件。
    以此带来不便，敬请谅解。
