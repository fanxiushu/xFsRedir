...
【强制卸载】
如果因为使用了不同版本，或者其他什么原因造成使用混乱。可以按照如下办法强制卸载：
** 注意：因为需要手动操作注册表，请慎重，如果因为操作不当造成任何损失自行承担。
打开注册表位置：
\HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services
从services中查找与本软件相关的服务，然后删除。
请慎重，确保确实是与本软件相关的服务才能删除。
删除之后，重启电脑即可。
重启电脑之后，再把相关的程序文件删除。
如果安装了 “扩展虚拟磁盘驱动”，不能删除“扩展虚拟磁盘驱动”的服务项，而是到设备管理器里边老老实实的删除对应驱动。
"扩展虚拟磁盘驱动"的服务项是“xfs_disk”，目前还没改变过名字。

【软件可能的使用场所：】
xFsRedir程序运行在各类windows平台
...
【更新历史记录：】
2024-01月更新：
1，给xFsRedir程序的NAT路由功能，添加了DHCP服务和TFTP服务， 这让NAT路由功能更接近硬件路由器。
     同时给NAT路由功能添加DNS转发功能，也就是局域网内可以直接设置网关IP当做DNS服务器IP。
     相比之前的6.0版本，只更新了NAT路由的DHCP和TFTP服务，
     也只更新了xFsRedir.exe文件和xfs_net.sys文件，如果对此需求不大，可以不用升级到6.1版本。

2022-10月更新：
1，添加 NSP HOOK功能，使得在本机代理的时候，能分辨哪个进程发起的域名访问，
      从而对特定的进程对域名的访问进行代理。
2，添加阻止程序运行的功能。
