...
如同访问本地文件系统一样。
xFsRedir包括三大部分功能：
   一，核心的功能是通过各种通讯协议把远程目录映射成本地目录。
   二，通过各种通讯协议把远程镜像文件或磁盘映射成本地磁盘。
   三，虚拟局域网，包括虚拟局域网节点和桥接到真实局域网。

他可以代替NetDrive以及虚拟磁盘类软件提供功能更强大的多文件服务器的集中管理和访问。
并且完全免费使用，本软件不隐含连接到某个固定服务器，
对于虚拟局域网功能，需要数据包转发服务器xfs_rdsvr，须自行去搭建配置。
因此也并不提供版本自动更新功能，若需要获新版本，或者发现BUG，
可以给本人发送邮件， mailto:fanxiushu@163.com，或者到CSDN以及GitHUB查询。
